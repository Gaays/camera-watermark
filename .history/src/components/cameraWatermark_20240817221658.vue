...
const handleDrawExif = (ctx, width, height) => {
  ...
  // 快门速度
  const exposureTime = output.value[4].value;
  // 光圈
  const fNumber = output.value[5].value;
  // ISO
  const iso = output.value[6].value;
  // 焦段
  const focalLength = output.value[7].value;
  const list = [focalLength, fNumber, exposureTime, iso]
  const text = list.join(' ')
  const textWidth = ctx.measureText(text).width;
  const textLeft = (width - textWidth);
  const textTop = (height + 20);
